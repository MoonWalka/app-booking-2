...
const useGenericDataFetcher = (entityType, fetchConfig = {}, options = {}) => {
  ...
  const buildQuery = useCallback(() => {
    ...
    if (currentEntreprise?.id) {
      console.log(`[useGenericDataFetcher] Filtrage par organizationId: ${currentEntreprise.id} pour ${entityType}`);
    ...
    try {
      ...
      if (stableFetchConfig.mode === 'single') {
        ...
        if (docSnap.exists()) {
          ...
          if (currentEntreprise?.id && docData.organizationId !== currentEntreprise.id) {
