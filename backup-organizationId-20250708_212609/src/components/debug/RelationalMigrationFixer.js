  ...
  const analyzeData = async () => {
    ...
    try {
      ...
      const [structuresSnap, personnesSnap, liaisonsSnap, unifiedSnap] = await Promise.all([
        getDocs(query(collection(db, 'structures'), where('organizationId', '==', currentEntreprise.id))),
        getDocs(query(collection(db, 'personnes'), where('organizationId', '==', currentEntreprise.id))),
        getDocs(query(collection(db, 'liaisons'), where('organizationId', '==', currentEntreprise.id))),
        getDocs(query(collection(db, 'contacts_unified'), where('organizationId', '==', currentEntreprise.id)))
      ]);
  ...
  const executeFix = async () => {
    ...
    try {
      ...
      if (!dryRun && analysis.structures.details.length > 0) {
        ...
        const unifiedSnap = await getDocs(query(collection(db, 'contacts_unified'), where('organizationId', '==', currentEntreprise.id)));
