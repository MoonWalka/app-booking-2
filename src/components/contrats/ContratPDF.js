...
import { Document, Page, Text, View, StyleSheet, Font, Image, Html } from '@react-pdf/renderer';
...
const ContratPDF = ({ template, concertData, programmateurData, artisteData, lieuData, entrepriseInfo }) => {
  console.log('ContratPDF rendu avec template:', template ? template.id : 'undefined');
  ...
  // Déterminer le format du modèle (ancien ou nouveau)
  const isNewFormat = template && template.bodyContent;
  const isOldFormat = template && template.sections && Array.isArray(template.sections);
  
  console.log('Format de modèle:', isNewFormat ? 'nouveau' : (isOldFormat ? 'ancien' : 'inconnu'));
  
  // Si aucun format valide n'est détecté, afficher un message d'erreur
  if (!isNewFormat && !isOldFormat) {
    console.error('Modèle de contrat invalide:', template);
  }
  ...
  // Fonction pour créer un style dynamique en fonction des paramètres du modèle
  const getHeaderStyle = () => {
    if (!isNewFormat) return styles.header;
    
    const headerHeight = template.headerHeight || 20;
    const headerBottomMargin = template.headerBottomMargin || 10;
    
    return {
      ...styles.customHeader,
      height: `${headerHeight}mm`,
      marginBottom: `${headerBottomMargin}mm`,
    };
  };
  
  const getFooterStyle = () => {
    if (!isNewFormat) return styles.footer;
    
    const footerHeight = template.footerHeight || 15;
    const footerTopMargin = template.footerTopMargin || 10;
    
    return {
      ...styles.customFooter,
      height: `${footerHeight}mm`,
      marginTop: `${footerTopMargin}mm`,
    };
  };
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        {/* En-tête */}
        {isNewFormat ? (
          /* Nouveau format avec en-tête personnalisé */
          <View style={getHeaderStyle()}>
            {template.logoUrl && (
              <Image src={template.logoUrl} style={styles.customHeaderLogo} />
            )}
            <Text>{parseHtmlToReactPdf(replaceVariables(template.headerContent))}</Text>
          </View>
        ) : (
          /* Ancien format avec en-tête standard */
          safeData.entreprise && (
        ...
        {isNewFormat ? (
          ...
          /* Format invalide ou manquant */
          <View style={styles.section}>
            <Text style={styles.sectionContent}>
              Ce document a été généré à partir d'un modèle incomplet ou manquant.
              Veuillez vérifier le format du modèle de contrat.
            </Text>
          </View>
        )}
        
        {/* Zone de signature - commune aux deux formats */}
        ...
        {/* Pied de page */}
        {isNewFormat ? (
          /* Nouveau format avec pied de page personnalisé */
          ...
          /* Ancien format avec pied de page standard */
