...
export default function useSimpleContactDetails(id) {
    ...
    try {
      ...
      if (contactEntity.lieuxIds?.length > 0 || contactEntity.lieuxAssocies?.length > 0) {
        ...
        if (lieuxLoaded.length === 0) {
          ...
          if (currentEntreprise?.id) {
            lieuxConstraints2.push(where('organizationId', '==', currentEntreprise.id));
          }
          lieuxQuery = query(
            collection(db, 'lieux'),
            ...lieuxConstraints2
  ...
  const fetchDatesAssocies = useCallback(async (contactEntity) => {
    try {
      ...
      if (contactEntity.datesIds?.length > 0 || contactEntity.datesAssocies?.length > 0) {
        ...
        if (currentEntreprise?.id) {
          datesConstraints.push(where('organizationId', '==', currentEntreprise.id));
        }
        ...
        if (datesLoaded.length === 0) {
          ...
          if (currentEntreprise?.id) {
            datesConstraints2.push(where('organizationId', '==', currentEntreprise.id));
          }
          datesQuery = query(
            collection(db, 'dates'),
            ...datesConstraints2
  ...
  const fetchArtistesAssocies = useCallback(async (contactEntity, datesData) => {
    try {
      ...
      if (currentEntreprise?.id) {
        artistesConstraints.push(where('organizationId', '==', currentEntreprise.id));
      }
