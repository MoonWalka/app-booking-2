...
const ContratGeneratorNew = ({ 
  ...
  useEffect(() => {
    const loadExistingContrat = async () => {
      ...
      try {
        console.log('[ContratGeneratorNew] Recherche d\'un contrat existant pour la date:', dateId);
    ...
    const loadDevisDuDate = async () => {
      ...
      try {
        ...
        if (devisList && devisList.length > 0) {
          ...
          console.log('[ContratGeneratorNew] Aucun devis trouvé pour cette date');
        }
      } catch (error) {
    ...
    if (date) {
      setContratData(prev => ({
        ...
        representations: {
          ...
          debut: date.dateDebut || date.date || '',
          fin: date.dateFin || date.date || '',
          salle: lieu?.nom || date.libelle || date.lieuNom || ''
        }
      }));
    }
  ...
  }, [structure, date, artiste, lieu, preContratData, entrepriseData]);
                ...
                <>
                  {devisData ? (
                    ...
                    <Alert variant="warning" className="small p-2">
                      ...
                      Aucun devis créé pour cette date
                    </Alert>
                  )}
                </>
              )}
