// Fichier de diagnostic pour les problèmes liés aux contacts
// src/utils/diagnostic/contactDiagnostic.js

/**
 * Utilitaire de diagnostic pour le suivi du cycle de vie des composants de contact
 */
 ...
 * @param {string} id - ID de l'entité (ex: contact)
...
// Exporter un objet de trace configuré pour les contacts
export const ContactTrace = {
  component: (name, id, props) => traceComponentMount(`Contact${name}`, id, props),
  hook: (name, id, params) => traceHookInit(`useContact${name}`, id, params),
  effect: (name, id, deps) => traceEffect(`useContactEffect${name}`, id, deps),
  firebase: (operation, id) => traceFirebaseOperation(operation, 'contacts', id),
  request: (name, stage, details) => traceAsyncRequest(`Contact${name}`, stage, details),
  render: (name, id, reason) => traceRender(`Contact${name}`, id, reason)
};
