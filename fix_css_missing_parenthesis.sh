...
# Script de correction des fallbacks CSS avec parenthèse manquante
# Ce script cible spécifiquement la syntaxe var(--variable, valeur sans parenthèse fermante
...
# Fonction pour corriger les fallbacks avec parenthèse manquante dans un fichier CSS
fix_missing_parenthesis() {
  local file="$1"
  local temp_file="$file.temp"
  local backup_success="$2"
  
  # Utiliser Perl pour la correction car les expressions régulières sont plus complexes
  # Cette expression recherche les cas où var( est suivi d'une variable, d'une virgule, 
  # d'une valeur, mais sans parenthèse fermante à la fin
  cat "$file" | perl -pe '
    s/var\((--[a-zA-Z0-9-_]+),\s*([^;{}()]+(rgba?\([^;{}()]+\))?[^;{}()]*);/var(\1);/g;
    s/var\((--[a-zA-Z0-9-_]+),\s*([^;{}()]+(rgba?\([^;{}()]+\))?[^;{}()]*)\)/var(\1)/g;
  ' > "$temp_file"
  ...
  if [ $? -eq 0 ]; then
    ...
    if ! diff -q "$file" "$temp_file" > /dev/null; then
      ...
      if [ $backup_success -eq 1 ]; then
        print_message "✅ Corrigé: $file (parenthèses manquantes corrigées)" "$GREEN"
      else
        print_message "⚠️ Corrigé sans sauvegarde: $file (parenthèses manquantes corrigées)" "$YELLOW"
      fi
      ...
      rm -f "$temp_file"
      print_message "ℹ️ Aucune parenthèse manquante trouvée dans: $file" "$BLUE"
      return 0
    fi
  else
...
main() {
  ...
  BACKUP_DIR="$PROJECT_DIR/css_parenthesis_backup_$TIMESTAMP"
  ...
  for file in $css_files; do
    ...
    # Corriger les fallbacks avec parenthèse manquante
    fix_missing_parenthesis "$file" "$backup_success"
    fix_success=$?
