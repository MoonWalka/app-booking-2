 ...
 */
export const useStructureFormOptimized = (structureId) => {
  const navigate = useNavigate();
  const isNewStructure = !structureId || structureId === 'nouveau';
  
  debugLog(`Initialisation du formulaire de structure optimisé: ${isNewStructure ? 'nouvelle structure' : `structure ${structureId}`}`, 'info', 'useStructureFormOptimized');
  ...
  const transformStructureData = (data) => {
    ...
    debugLog('Données transformées avant sauvegarde', 'debug', 'useStructureFormOptimized', transformedData);
  ...
  const onErrorCallback = useCallback((error) => {
    ...
    debugLog(error, 'error', 'useStructureFormOptimized');
  }, [isNewStructure]);
...
export default useStructureFormOptimized;