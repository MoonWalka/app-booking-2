  ...
  const create = useCallback(async (data, customId = null) => {
    ...
    try {
      ...
      let entityData = {
        ...
        ...(currentEntreprise?.id && { organizationId: currentEntreprise.id })
      };
      ...
      if (entityType === 'contacts') {
        ...
        entityData = {
          ...
          // Champs obligatoires
          organizationId: entityData.organizationId || currentEntreprise?.id,
  ...
  const update = useCallback(async (id, data) => {
    ...
    try {
      ...
      let updateData = {
        ...
        ...(currentEntreprise?.id && !data.organizationId && { organizationId: currentEntreprise.id })
      };
      ...
      if (entityType === 'contacts') {
        ...
        updateData = {
          ...
          // Champs obligatoires
          organizationId: updateData.organizationId || currentEntreprise?.id,
          updatedAt: new Date(),
        ...
        if (!updateData.organizationId) {
          ...
          updateData.organizationId = currentEntreprise?.id;
          if (!updateData.organizationId) {
            throw new Error('❌ organizationId OBLIGATOIRE pour les contacts');
          }
        }
    ...
    try {
      ...
      if (!skipOrganizationFilter) {
        ...
        if (currentEntrepriseId) {
          constraints.push(where('organizationId', '==', currentEntrepriseId));
          if (enableLogging) {
            console.log(`🔒 [useGenericAction] Filtre organizationId ajouté: ${currentEntrepriseId} pour ${entityType}`);
          }
        }
      }
