...
/**
 * Version refactorisée de ConcertDetails utilisant les nouveaux composants anti-boucles
...
const ConcertDetailsRefactored = () => {
  ...
  // Debug logs
  useEffect(() => {
    console.log('🎵 ConcertDetailsRefactored monté avec ID:', id);
  }, [id]);
  
  // Utiliser le hook sécurisé avec profondeur 1 (charge les relations directes seulement)
  ...
  // Plus de debug
  useEffect(() => {
    console.log('📊 État du concert:', { loading, error, concert });
  }, [loading, error, concert]);
  
  // Debug logs pour les données
  useEffect(() => {
    console.log('🎵 État du chargement:', { loading, error, concert });
  }, [loading, error, concert]);
    ...
    <div className="concertDetailsContainer">
          ...
          <div className="row">
            ...
            <div className="col-md-6">
              <div className="mb-3">
                ...
                <div>{concert.lieu.capacite ? `${concert.lieu.capacite} personnes` : 'Non spécifiée'}</div>
              </div>
              <div className="mb-3">
      ...
      {concert.contact && (
        <Card className="detailsCard">
          ...
          <RelationCard
            entity={concert.contact}
            type="contact"
            onClick={(id) => navigate(`/contacts/${id}`)}
            showBadge={false}
          />
        </Card>
      )}
