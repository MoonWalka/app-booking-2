...
import { useAdresseValidation as useAdresseValidationContact } from '@/hooks/contacts';
...
const ContactAddressSection = ({
  ...
  const adresseData = isEditing 
    ...
    : {
        ...
        ville: contact?.structureVille || '',
        pays: contact?.structurePays || 'France'
      };
  ...
  const { 
    ...
    validateAdresse,
    selectSuggestion
  } = useAdresseValidationContact(adresseData);
  ...
  const getFullAddress = () => {
    if (!contact) return 'Adresse non spécifiée';
    
    const addrParts = [];
    
    if (contact.structureAdresse) addrParts.push(contact.structureAdresse);
    
    let cityPart = '';
    if (contact.structureCodePostal) cityPart += contact.structureCodePostal;
    if (contact.structureVille) {
      if (cityPart) cityPart += ' ';
      cityPart += contact.structureVille;
    }
    
    if (cityPart) addrParts.push(cityPart);
    if (contact.structurePays && contact.structurePays !== 'France') {
      addrParts.push(contact.structurePays);
    }
