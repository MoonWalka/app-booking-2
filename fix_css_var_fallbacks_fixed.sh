...
fix_fallbacks() {
  ...
  # Expression régulière corrigée pour trouver les fallbacks CSS
  # Utilise une expression qui capte tout après la virgule jusqu'à la dernière parenthèse fermante
  local pattern="var\\((-{2}[a-zA-Z0-9_-]+),\\s*[^\\)]+\\)"
  local replacement="var\\(\\1\\)"
  
  # Vérifier si le fichier contient des fallbacks
  if grep -E "$pattern" "$file" > /dev/null; then
    # Compter le nombre de fallbacks avant modification
    ...
    if [ $? -eq 0 ]; then
      # Compter le nombre de fallbacks restants après modification
