...
class DateService {
  ...
  async getDatesByStructureId(entrepriseId, structureId) {
    try {
      ...
      console.log('  📋 Requête 1: where organizationId ==', entrepriseId, 'AND structureId ==', structureId);
      let datesQuery = query(
        collection(db, 'dates'),
        where('organizationId', '==', entrepriseId),
      ...
      if (dates.length === 0) {
        console.log('  📋 Requête 2: where organizationId ==', entrepriseId, 'AND organisateurId ==', structureId);
        datesQuery = query(
          collection(db, 'dates'),
          where('organizationId', '==', entrepriseId),
