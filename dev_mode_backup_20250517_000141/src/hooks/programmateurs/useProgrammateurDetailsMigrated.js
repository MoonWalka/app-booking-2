  ...
  useEffect(() => {
    const fetchLieuxAssocies = async () => {
      ...
      try {
        ...
        console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - Chargement des lieux pour programmateur ${id}`);
        ...
        if (details.entity.lieuxIds?.length > 0 || details.entity.lieuxAssocies?.length > 0) {
          ...
          console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - ${lieuxRefs.length} références de lieux trouvées dans le programmateur`);
          ...
          console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - ${validLieux.length} lieux chargés depuis les références`);
          
          setLieux(validLieux);
        } else {
          // Si pas de référence directe, chercher par référence inverse
          console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - Pas de références directes, recherche par référence inverse`);
          ...
          if (lieuxLoaded.length === 0) {
            console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - Pas de résultat avec 'programmateurs', essai avec 'programmateurId'`);
            lieuxQuery = query(
              collection(db, 'lieux'),
          ...
          console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - ${lieuxLoaded.length} lieux trouvés par référence inverse`, { 
        ...
        console.error('[ERROR] useProgrammateurDetailsMigrated - Erreur lors du chargement des lieux:', error);
  ...
  useEffect(() => {
    const fetchConcertsAssocies = async () => {
      ...
      try {
        ...
        console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - Chargement des concerts pour programmateur ${id}`);
        ...
        if (details.entity.concertsIds?.length > 0 || details.entity.concertsAssocies?.length > 0) {
          ...
          console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - ${concertRefs.length} références de concerts trouvées dans le programmateur`);
          ...
          console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - ${validConcerts.length} concerts chargés depuis les références`);
          ...
          if (concertsLoaded.length === 0) {
            console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - Pas de résultat avec 'programmateurId', essai avec 'programmateurs'`);
            concertsQuery = query(
              collection(db, 'concerts'),
              where('programmateurs', 'array-contains', id)
            );
          ...
          console.log(`[DIAGNOSTIC] useProgrammateurDetailsMigrated - ${concertsLoaded.length} concerts trouvés par référence inverse`, { 
        ...
        console.error('[ERROR] useProgrammateurDetailsMigrated - Erreur lors du chargement des concerts:', error);
