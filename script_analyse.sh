...
echo -e "${BLUE}=== Analyse des dépendances circulaires ===${NC}\n"

# Vérifier si madge est installé
if ! command -v madge &> /dev/null; then
    echo -e "${YELLOW}Installation de madge...${NC}"
    npm install -g madge
fi

# Créer un dossier pour les résultats
REPORT_DIR="circular_deps_analysis"
mkdir -p "$REPORT_DIR"

# Identifier les dépendances circulaires
echo -e "${YELLOW}Identification des dépendances circulaires...${NC}"
CIRCULAR_DEPS=$(madge --circular --extensions js,jsx src/)

if [ -z "$CIRCULAR_DEPS" ]; then
    echo -e "${GREEN}✓ Aucune dépendance circulaire détectée${NC}"
    exit 0
fi

# Enregistrer les dépendances circulaires
echo "$CIRCULAR_DEPS" > "$REPORT_DIR/circular_deps.txt"
echo -e "${RED}Dépendances circulaires détectées:${NC}"
echo "$CIRCULAR_DEPS"

# Générer un graphique des dépendances
echo -e "\n${YELLOW}Génération d'un graphique des dépendances...${NC}"
madge --image "$REPORT_DIR/dependency-graph.png" --extensions js,jsx src/
echo -e "${GREEN}✓ Graphique généré: $REPORT_DIR/dependency-graph.png${NC}"

# Analyser les dépendances et proposer des solutions
echo -e "\n${BLUE}=== Analyse des dépendances circulaires ===${NC}"

# Créer un script JavaScript pour analyser les dépendances
cat > "$REPORT_DIR/analyze_circular_deps.js" << 'EOL'
const fs = require('fs');
...
echo -e "${YELLOW}Analyse des dépendances circulaires et génération de solutions...${NC}"
node "$REPORT_DIR/analyze_circular_deps.js"

echo -e "\n${GREEN}✓ Analyse terminée${NC}"
echo -e "${BLUE}Consultez $REPORT_DIR/solutions.md pour des recommandations détaillées${NC}"
echo -e "${YELLOW}Après avoir appliqué les solutions, vérifiez à nouveau les dépendances circulaires avec:${NC}"
echo -e "madge --circular src/"
