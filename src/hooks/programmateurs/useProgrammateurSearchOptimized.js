...
export const useProgrammateurSearchOptimized = ({
  ...
  useEffect(() => {
    const loadAllProgrammateurs = async () => {
      if (!hasLoadedInitialData && searchHook.results.length === 0 && !searchHook.isSearching) {
        debugLog('[useProgrammateurSearchOptimized] Chargement initial des programmateurs', 'info');
        ...
        try {
          ...
          debugLog(`[useProgrammateurSearchOptimized] ${transformedProgrammateurs.length} programmateurs chargés`, 'info');
        } catch (error) {
          debugLog(`[useProgrammateurSearchOptimized] Erreur lors du chargement initial: ${error.message}`, 'error');
          setError(error);
        }
      }
    };
  ...
  useEffect(() => {
    if (effectiveResults.length > 0) {
      ...
      // Log de debug
      debugLog(`[useProgrammateurSearchOptimized] Résultats triés par ${sortField} ${sortDirection}`, 'debug', 'search');
      ...
      if (searchFilters && searchFilters.length > 0) {
        ...
        debugLog(`[useProgrammateurSearchOptimized] Résultats filtrés: ${filteredResults.length}`, 'debug', 'search');
      }
